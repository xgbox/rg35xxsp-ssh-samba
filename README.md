...
Samba unfortunately does not seem to be packaged with the stock OS, so _samba_enable.sh_ will do an apt-get command to pull the necessary packages off the Internet in order to install it. Depending on your Internet speeds, that may take a few minutes. Samba will stay enabled throughout reboots and shutdown. Once the script finishes, you should be able to start accessing it through Windows by opening your start menu and typing in \\\<Your RG35XXSP's IP>\ during which point you should be prompted for credentials, which are once again root and root. 
